...
import client2.ClientThread;
import client2.DataBuffer;
import client2.entity.MyCellRenderer;
import client2.entity.OnlineUserListModel;
import client2.util.ClientUtil;
import client2.util.JFrameShaker;
...
public class ChatFrame extends JFrame{
    ...
    public void init(){
        ...
        otherInfoLbl = new JLabel("当前状态：群聊状态");
            ...
            public void actionPerformed(ActionEvent e) {
                if(rybqBtn.isSelected()){
                    ...
                    if(null == selectedUser){
                        otherInfoLbl.setText("当前状态：选择私聊对象(选中在线用户列表中某个用户进行私聊)");
                    }else if(DataBuffer.currentUser.getId() == selectedUser.getId()){
                        otherInfoLbl.setText("当前状态：系统不允许与自己进行私聊");
                    }else{
                        otherInfoLbl.setText("当前状态：与 "+ selectedUser.getNickname()
                                + "私聊中");
                    }
                }else{
                    otherInfoLbl.setText("当前状态：群聊状态");
                }
            }
        });
